...
void mult_matrix(double* A, double* B, double* C, int lower, int upper, int d) {
	...
	for(i = lower; i < upper; i++) {
		for(j = 0; j < d; j++) {
			for(k = 0; k < d; k++) {
					C[d*i+j] += A[d*i+k] * B[d*j+k]; //Changed to fetch elements from matrix B. B is stored in Column Major Form
			}
		}
	}
}
...
int main(int argc, char *argv[])
{
	...
	//Check how many arguments and accordingly se thte global variables
	if (argc == 2) {
		j = atoi(argv[1]);
		setSizeAndNofRuns(j);
	...
	for(thread=1;thread<=8;thread++) {
		...
		for (d = dimension ; d < dimension+1; d=d+128 ) {
			...
			for (run = 0 ; run < NO_OF_RUNS; run++) {
				...
				//Go for initialize the Matrices. Matrices are in row major form.
				gettimeofday(&begin, NULL); /* returns the wall clock time */
				for(i = 0; i < d; i++) {
					for(j = 0; j < d; j++) {
						A[d*i+j] = (rand()/(RAND_MAX - 1.0));
						B[d*j+i] = (rand()/(RAND_MAX - 1.0)); //values are stored in column major form in matrix B
						C[d*i+j] = 0.0;
					}
				}
				gettimeofday(&end, NULL);
